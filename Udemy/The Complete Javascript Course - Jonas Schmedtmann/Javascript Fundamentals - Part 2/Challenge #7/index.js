...
//  height (Mark Miller and John Smith). Name these objects as mark and john, and their properties exactly as fullName, mass and height.
...
const mark = {
    ...
    calcIBM: function() {
        this.ibm = (this.mass / (this.height ** 2)).toFixed(2)
        return this.ibm
    }
}
...
console.log(john.calcIBM() > mark.calcIBM() 
    ? `${john.fullName} BMI(${john.calcIBM()}) is higher than ${mark.fullName} (${mark.calcIBM()})`
    : `${mark.fullName} BMI(${mark.calcIBM()}) is higher than ${john.fullName} (${john.calcIBM()})`)